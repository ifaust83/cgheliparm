...
<strong> Note: </strong> MDAnalysis must be installed together with the NumPy module with python 2.7. Currently MDAnalysis does not support python3, which hinders cgHeliParm supporting python3 for the moment. Once there is a compatible python3, we will have a python3-based version of cgHeliParm. Also PDB files can be analyzed using the script cgHeliParm_PDB.py. When running cgHeliParm.py, look for the library_path that links to the 'data' folder in your system and modify it accordingly.
